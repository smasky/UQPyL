...
# import copy
# class SWAT_CUP_Parallel():
#     def __init__(self, work_path, para_file_name, observe_file_name, swat_exe_path, rch_id, n_parallel=4):
#         self.n_parallel=n_parallel
#         self.work_temp_dir = tempfile.mkdtemp()
#         self.original_work_path=work_path
#         self.swat_exe_path=swat_exe_path
        
#         swat_cup=SWAT_CUP(work_path=work_path, para_file_name=para_file_name, observe_file_name=observe_file_name, swat_exe_path=swat_exe_path, rch_id=rch_id)
        
#         swat_cup_parallels= [copy.deepcopy(swat_cup) for _ in range(self.n_parallel)]
        
#         for i in range(self.n_parallel):
#             swat_cup_parallels[i].work_path=os.path.join(self.work_temp_dir, f'instance_{i}')
        
#     def copy_and_run(self, instance_id, inputs_queue):
#         temp_dir=self.work_temp_dir
#         temp_folder_path = os.path.join(temp_dir, f'instance_{instance_id}')
#         shutil.copytree(self.original_work_path, temp_folder_path)
        
#         outputs=[]
#         while not inputs_queue.empty():
#             try:
#                 intput_index, input_data = inputs_queue.get_nowait()
#                 Y=SWAT_CUP_Parallel[instance_id].evaluate(input_data.reshape(1,-1))
#                 outputs.append(1)
...
#         inputs_queue = manager.Queue()
        
        
#         results=[]
#         for input_index, input_data in enumerate(X):
#             inputs_queue.put((input_index, input_data))

#         with multiprocessing.Pool(processes=self.num_instances) as pool:
#             async_results = [pool.apply_async(self.copy_and_run_instance, args=(i, inputs_queue)) for i in range(self.num_instances)]
#             pool.close()
#             pool.join()
...
#     swat_pro=SWAT_CUP(work_path="H:\SiHuRiver\model\FuTIanSi001\Scenarios\Test\TxtInOut",
#                         para_file_name="paras_infos.txt",
...
#     lhs=LHS(problem=problem)
#     X=lhs.sample(12, problem.n_input)

#     parallel=SWAT_CUP_Parallel(work_path="H:\SiHuRiver\model\FuTIanSi001\Scenarios\Test\TxtInOut",
#                         para_file_name="paras_infos.txt",
#                         observe_file_name="observed.txt",
#                         swat_exe_path="swat_64rel.exe",
#                         rch_id=40, n_parallel=3)
#     parallel.run(X)
...
X=np.loadtxt('./res_X.txt')
Y=np.loadtxt('./res_Y.txt')
# # X=morris_method.sample(128)
# # Y=problem.evaluate(X)
