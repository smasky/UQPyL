...
os.chdir('./examples')
import pandas as pd
database=pd.read_excel("./database_svr.xlsx", index_col=0)
#tmp
import numpy as np
    ...
    for dim in dimensions:
        for sample in samples:
            for time in range(2):
                ...
                print(index, time,dim, sample, r2, rank)
                database.loc[len(database)]=[problem.__class__.__name__, 'SVR-M3', index, time,dim, sample, r2, rank]
            index+=1
    database.to_excel("./database_svr.xlsx")
