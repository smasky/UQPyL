...
surrogate=["RBF-CB", "RBF-LN", "RBF-TPS", "RBF-MLP"]
...
#             res=database[(database['surrogate'] == sur) & (database['dimensions'] == dim) & (database['samples'] == sample)]['r_square'].to_numpy()
#             res[res<0]=0
#             count=np.sum(res==0)
#             print(sur, "dim:", dim,"sample:", sample,"mean:", np.mean(res), "std:", np.std(res))
   
for key, problem in benchmarks.items():
    score_2={"RBF-CB": 0, "RBF-LN": 0, "RBF-TPS": 0, "RBF-MLP": 0}
    score_1={"RBF-CB": 0, "RBF-LN": 0, "RBF-TPS": 0, "RBF-MLP": 0}
    for dim in dimensions:
        for sample in samples:
