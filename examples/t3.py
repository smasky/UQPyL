...
if __name__ == "__main__":
    ...
    with pm.Model() as model:
        ...
        idata = pm.sample(
            draws=10000,                      # 和你完全一样
            tune=1000,                        # DREAM-ZS 也会用调优阶段初始化 population
            chains=4,                         # DREAM-ZS 推荐 ≥3 条链（和你保持一致）
            step=pm.DEMetropolisZ(),          # ← 就是 DREAM-ZS 算法！
            random_seed=42,
            return_inferencedata=True,
            progressbar=True
        )

    # 保存成 DREAM-ZS 专属文件（方便和你自定义 DREAM-ZS .nc 对比）
    # idata.to_netcdf("pymc_banana_DREAMZ.nc")

    # ==================== 计算 ESS（和你之前代码 100% 一样） ====================
    ess  = az.ess(idata, var_names="theta")
    rhat = az.rhat(idata, var_names="theta")

    print("\n=== PyMC DREAM-ZS 的原始 ESS ===")
    print(ess)

    mean_ess = float(ess["theta"].mean())
    print(f"\n=== Mean ESS (PyMC DREAM-ZS Banana): {mean_ess:.0f}   ← 直接和你自定义 DREAM-ZS 对比！")
