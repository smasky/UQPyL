...
for dim in dimensions:
    for sample in samples:
        for time in range(20):
            ...
            train_X=lhs.sample(100, problem.n_input, random_seed=seed_train[index, time])
            train_Y=problem.evaluate(train_X)
            
            test_X=lhs.sample(100, problem.n_input, random_seed=seed_test[index, time])
            ...
            database.loc[len(database)]=['Sphere', 'RBF-TPS', dim, sample, r2, rank]
        index+=1
