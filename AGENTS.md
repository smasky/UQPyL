...
- 命名分层原则：内部实现字段、方法、运行态对象默认沿用仓库现有驼峰风格；对外暴露的返回协议、reader 汇总结果、导出 payload 优先使用 `snake_case`。不要为了导出格式去反推内部实现统一成 `snake_case`。
- 本轮审查、TODO 与交接记录按用户要求放在 `agent/`；接手时先读 `agent/0918.md` 和 `agent/TODO.md`。已有 `.codex/` 设计笔记保留追溯用途，以当前代码、测试及最新交接记录为准，避免重复堆积历史文档。
- 目前不需要向后兼容，正在开发阶段。
